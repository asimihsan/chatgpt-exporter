...
function mergeArrays<T>(target: T[] | undefined, source: T[] | undefined): T[] | undefined {
...
function mergeUniqueStrings(target: string[] | undefined, source: string[] | undefined): string[] | undefined {
    if (!source || source.length === 0) return target
    return Array.from(new Set([...(target ?? []), ...source]))
}

function mergeContinuationMetadata(target: ConversationNodeMessage, source: ConversationNodeMessage): void {
    if (!source.metadata) return

    target.metadata ??= {}
    target.metadata.content_references = mergeArrays<ContentReference>(
        target.metadata.content_references,
        source.metadata.content_references,
    )
    target.metadata.citations = mergeArrays<Citation>(
        target.metadata.citations,
        source.metadata.citations,
    )
    target.metadata.n7jupd_crefs = mergeArrays<ContentReference>(
        target.metadata.n7jupd_crefs,
        source.metadata.n7jupd_crefs,
    )
    target.metadata.content_references_by_file = mergeRecordArrays<ContentReference>(
        target.metadata.content_references_by_file,
        source.metadata.content_references_by_file,
    )
    target.metadata.n7jupd_crefs_by_file = mergeRecordArrays<ContentReference>(
        target.metadata.n7jupd_crefs_by_file,
        source.metadata.n7jupd_crefs_by_file,
    )
    target.metadata.exported_generated_file_ids = mergeUniqueStrings(
        target.metadata.exported_generated_file_ids,
        source.metadata.exported_generated_file_ids,
    )
}
